...
CHAT_SRC_PATH = Path(__file__).parent.parent.parent / "corvus" / "gateway" / "run_executor.py"


def _find_ws_routing_block(source: str) -> str:
    """Find the emitter.emit block for routing_decision in run_executor.

    Returns the text block from 'runtime.emitter.emit' through the closing ')'.
    run_executor receives runtime as a parameter and calls
    'await runtime.emitter.emit(...)' directly,
    while webhooks.py uses a fire-and-forget helper.
    """
    # Match runtime.emitter.emit( <optional newline+whitespace> "routing_decision" ... )
    # across multiple lines (the call is formatted with line breaks)
    pattern = r'runtime\.emitter\.emit\(\s*["\']routing_decision["\'].*?\)'
    match = re.search(pattern, source, re.DOTALL)
    assert match is not None, "routing_decision emit block not found in run_executor.py"
...
class TestRoutingDecisionEmit:
    ...
    def test_routing_decision_emitted_in_websocket_path(self):
        """run_executor contains a runtime.emitter.emit('routing_decision', ...) call."""
    ...
    def test_websocket_emit_is_awaited(self):
        """The emitter.emit call in run_executor is awaited (async context)."""
        source = CHAT_SRC_PATH.read_text()
        # Find the await keyword immediately before the emitter.emit call
        pattern = r'await\s+runtime\.emitter\.emit\(\s*["\']routing_decision["\']'
        match = re.search(pattern, source, re.DOTALL)
        assert match is not None, "routing_decision emit should be awaited in run_executor"
    ...
    def test_both_webhook_and_websocket_emit_routing_decision(self):
        ...
        assert "routing_decision" in block, "run_executor.py missing routing_decision emit"
